...
public class ThalmicMyo : MonoBehaviour {
    ...
    void Start() {
        // EM: Coroutine
        StartCoroutine("CalculateAccelerometer");
    }

    // EM: Variables
    public float initVel;
    public bool canCalc = true;

    // EM: Calculo Accelerometer
    IEnumerator CalculateAccelerometer() {
        if(pose == Pose.Fist) {
            initVel = accelerometer.z;
        }
    ...
    void Update() {
        lock (_lock) {
            ...
            if(pose == Pose.Rest) {
                animator.SetInteger("pose-anim", 1);
            }

            if(pose == Pose.Fist) {
                animator.SetInteger("pose-anim", 0);
            }
            ...
            if(pose != Pose.Fist) {
                ...
                // canCalc = false;
                // var initVelocity = initVel;
                // var endVelocity = accelerometer.z;
                // var resultVel = endVelocity - initVelocity;
                // Debug.Log(resultVel);
            }
